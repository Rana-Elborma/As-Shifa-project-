...
export function PatientRegistration({ onBackToLanding, onRegistrationSuccess }: PatientRegistrationProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data: signUpData, error: signUpError } = await supabase.auth.signUp({
      ...
      options: {
        data: { name: formData.fullName, role: selectedRole, phone: formData.phone, date_of_birth: formData.dateOfBirth },
      },
    });
                ...
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {(['patient', 'doctor', 'admin'] as UserRole[]).map((role) => {
                        const icons = { patient: User, doctor: Stethoscope, admin: UserCog };
                  ...
                  <div className="backdrop-blur-xl bg-gradient-to-r from-green-50/80 to-blue-50/80 border border-green-200/60 rounded-2xl p-4 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="flex items-center gap-1.5 text-green-700"><CheckCircle2 className="w-3.5 h-3.5" /><span>256-bit AES Encryption</span></div>
                      <div className="flex items-center gap-1.5 text-green-700"><CheckCircle2 className="w-3.5 h-3.5" /><span>HIPAA Compliant</span></div>
                      <div className="flex items-center gap-1.5 text-green-700"><CheckCircle2 className="w-3.5 h-3.5" /><span>2FA Authentication</span></div>
                      <div className="flex items-center gap-1.5 text-green-700"><CheckCircle2 className="w-3.5 h-3.5" /><span>Audit Logging</span></div>
                    </div>
                  </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-blue-700">
                          Your password is hashed and stored securely. We use industry-leading security to protect your account and medical data.
                        </p>
                      </div>
                    </div>
                  </div>
